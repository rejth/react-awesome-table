...
const TableBody = React.forwardRef(
  (props: ITableBodyProps, ref: React.ForwardedRef<HTMLDivElement>): JSX.Element => {
    const {
      columns,
      rows,
    ...
    const renderRow = (row: ITableRow) => {
      ...
      const simpleCells = columns.reduce(
        (cols: React.ReactNode[], column: ITableColumn, index: number) => {
      ...
      const stickyCells = columns.reduce(
        (cols: React.ReactNode[], column: ITableColumn, index: number) => {
          if (column.isShow && column.isFixed) {
            cols.push(renderBodyCell(column, row, isBlocked, index));
          }
        ...
        >
          ...
          {isExpanded && row?.expandedRowRender && (
            <div className="table-body-row-expanded">
              <div className="table-body-row-expanded-content">{row?.expandedRowRender(row)}</div>
            </div>
          )}
        </div>
      );
